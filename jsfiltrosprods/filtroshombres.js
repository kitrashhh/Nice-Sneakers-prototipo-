...
document.getElementById("aplicarfiltros-hombres").addEventListener("click", async () => {
...
async function filtrarProds(tallas, colores, marca) {
  ...
  //crear consulta
  const consula = query(prodsRef, where("categoria", "array-contains", "Hombres"));
  ...
  document.getElementById("Hombress").innerHTML = ""; //limpia la seccion
  ...
  filtradoss.forEach((doc) => {
    ...
    document.getElementById("Hombress").appendChild(item);
...
document.getElementById("limpiarfiltros-hombres").addEventListener("click", async () => { //el async define una funcion para usar await cosas que pueden tardar en cargar como firestore
