...
async function filtrarProds(tallas, colores, marca) {
  ...
  //crear consulta
  const consula = query(prodsRef, where("categoria", "array-contains", "Mujeres"));
  ...
  document.getElementById("Mujeress").innerHTML = ""; //limpia la seccion
  ...
  filtradoss.forEach((doc) => {
    ...
    document.getElementById("Mujeress").appendChild(item);
...
document.getElementById("limpiarfiltros-mujeres").addEventListener("click", async () => { //el async define una funcion para usar await cosas que pueden tardar en cargar como firestore
