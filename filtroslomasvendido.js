...
document.getElementById("aplicarfiltros-vendido").addEventListener("click", async () => {
...
async function filtrarProds(tallas, colores, marca) {
  ...
  //crear consulta
  const consula = query(prodsRef, where("categoria", "array-contains", "Lo más vendido"));
  ...
  document.getElementById("LoMasVendido").innerHTML = ""; //limpia la seccion

  //recorrer los productos y mostrarlos
  //LO MAS VENDIDO
  filtradoss.slice(0, 5).forEach((doc) => { //el .slice toma los primeros 5 productos del array filtradoss
    ...
    document.getElementById("LoMasVendido").appendChild(item);
...
document.getElementById("limpiarfiltros-vendido").addEventListener("click", async () => { //el async define una funcion para usar await cosas que pueden tardar en cargar como firestore
